...
library(readr)
p <- arg_parser("Run SGSeq::getSGFeatureCounts for a single sample")
...
p <- add_argument(p, 'splice_graph', type = "character", help = 'Splice graph')
...
# Load the splice graph as `splice_graph_features`
load(argv$splice_graph)

# Run getSGFeatureCounts
sg_feature_counts <- 
  SGSeq::getSGFeatureCounts(
    sample_info = bam_info.df,
    features = splice_graph_features,
    min_anchor = 1,
    counts_only = FALSE,
    verbose = TRUE,
...
save(sg_feature_counts, file = argv$out_file)
